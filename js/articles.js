...
import { bestArticles } from "./data.js";


// шаблон
const templateArticles = document.querySelector('.template__articles').innerHTML;
const articlesData = JSON.parse(bestArticles);

articlesData.forEach(elem => {
    document.querySelector('.itc-slider__items').insertAdjacentHTML('beforeend', (new Templater(templateArticles)).compile({
        articleId: elem.id,
        articleImg: elem.img,
        articleName: elem.name,
        articlePrevious: elem.previous,
    }));
});
